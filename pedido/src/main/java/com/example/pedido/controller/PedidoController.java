...
public class PedidoController {
    ...
    @Operation(summary = "Crear un nuevo pedido", description = "Registra un nuevo pedido en el sistema, calcula los montos y gatilla las validaciones de stock")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pedido creado exitosamente", content = @Content(schema = @Schema(implementation = PedidoDTO.class))),
            @ApiResponse(responseCode = "400", description = "Datos de entrada inválidos o inconsistentes")
    })
    @PostMapping
    public PedidoDTO crear(@Valid @RequestBody PedidoDTO pedidoDTO) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Pedido encontrado correctamente", content = @Content(schema = @Schema(implementation = PedidoDTO.class))),
    ...
    @Operation(summary = "Listar todos los pedidos", description = "Retorna una lista completa con todos los pedidos registrados en el sistema")
    @ApiResponses(value = {
